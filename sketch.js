...
var slime, guard, bush, lonelyTree, THEGREATANCIENTBUSHOFBUSHANIA_SPEAKS
...
THEGREATANCIENTBUSHOFBUSHANIA_SPEAKS=loadSound("THEGREATSPLAT");
...
function draw() {
  ...
  if(gameState=="Start"){
    textSize(45);
    text("FIND A BUSH AND HIDE!! PRESS SPACE TO CONTINUE!!", 135,155);
    if(keyDown ("SPACE")){
      gameState="Play"
  ...
  if(gameState=="Play"){
    ...
    slimeMovement();
    if(slime.isTouching(bush1)){
      THEGREATANCIENTBUSHOFBUSHANIA_SPEAKS.play()
    }
  if(time1-time>=10){
    time1=0;
    time=0;
